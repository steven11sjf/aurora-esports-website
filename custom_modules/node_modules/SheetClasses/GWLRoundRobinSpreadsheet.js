...
module.exports = class GWLRoundRobinSpreadsheet extends GWLSpreadsheet {
	...
	_storeherostats(rows,league,totals,playerTotals) {
		return new Promise((resolve,reject) => {
			var json = '{"stats":[';
			if(rows && rows.length && league && league.length && totals && totals.length && playerTotals && playerTotals.length) {
				rows.map((row) => {
				...
				playerTotals.map((row) => {
					...
					json += `"damage":"${row[4]}","deaths":"${row[5]}",`;
					json += `"healing":"${row[6]}","blocked":"${row[7]}",`;
					json += `"timeplayed":"${row[8]}","team":"All Heroes"},`;
				});
	...
	batchGetAll() {
		return new Promise((resolve,reject) => {
			...
			ranges.push('HeroStats!V2:AD2000');
